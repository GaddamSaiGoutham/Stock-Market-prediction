...
t=pd.read_csv('~/stocks_prediction/tharun/^BSESN.csv',nrows = 5000)
...
for i in range(30,4903) :              #taking input data from row 30 till 4903
 ...
 for c in range(i-30,i) :
  ...
  average3 = c1/30
 #print average3
 input_array = input_array + [High1,Low1,Close1,Close2,Close3,Close4,Close5]
 if count == 1 :
    d = input_array	
 else :
...
np.random.seed(7)
model=Sequential()
model.add(Dense(7,input_dim=7,activation='sigmoid'))   # 240
...
#model.add(Dense(7,activation='tanh'))     #121

model.add(Dense(7 ,activation='relu',use_bias=True))   #16
#model.add(Dropout(0.1))
#model.add(Dense(16,activation='relu',use_bias=True))
model.add(Dense(1,activation='relu'))
sgd = optimizers.SGD(lr=0.002, decay=1e-7, momentum=0.9, nesterov=True)
model.compile(loss='mse', optimizer='sgd',metrics=['accuracy'])
b = d[0:4273]

model.fit(b,output_array,epochs=600,batch_size=20)
...
#print g.type
z = 0
for k in range(4303,4903) :                 #0.97606449 for 700 testin
 r = ((x[k])-(f[k-4303]))**2
 z = z + abs(r)
...
plt.figure()
plt.plot(x[4104:]*t.Close[index]-f*t.Close[index])
plt.xlabel('Days')
plt.ylabel('Error')
plt.show()'''
